...
// ┌───────────────────────────────────────────────────────────────────────────
// │ SECTION 1: SHARP LUT (mV → mm) — PER-SENSOR
// │
// │ Each of the 4 Sharp units has its own mV-vs-distance curve. They differ
// │ enough — LEFT especially — that one shared LUT would give >25 mm error
// │ (which would wreck the steering CTE). So each sensor gets its own LUT.
// │
// │ Calibrated 2026-05-22, 3.3V supply, straight-on (wall perpendicular to the
// │ beam). Straight-on is CORRECT even for the L/R sensors that are mounted
// │ angled ~60° forward: the LUT is the sensor's intrinsic mV→beam-path curve,
// │ independent of mounting. The angle only scales the downstream geometry,
// │ which is handled empirically (measured nominal + tuned steering gains).
// │
// │ Re-calibrate: run 'k', pick the sensor, paste its block over the matching
// │ SHARP_LUT_* array, reflash. Order = mV high→low (near→far); the mV column
// │ MUST be strictly decreasing, and every array MUST have SHARP_LUT_SIZE rows.
// └───────────────────────────────────────────────────────────────────────────

struct _SharpPt { uint16_t mv; uint16_t mm; };

// ── RIGHT sensor ──
static constexpr _SharpPt SHARP_LUT_RIGHT[] = {
    {1650,  20}, {1374,  30}, {1106,  40}, { 919,  50}, { 789,  60},
    { 700,  70}, { 608,  80}, { 534,  90}, { 479, 100}, { 380, 120}, { 281, 140},
};
// ── FRONT-RIGHT sensor ──
static constexpr _SharpPt SHARP_LUT_FRONT_R[] = {
    {1919,  20}, {1472,  30}, {1165,  40}, {1012,  50}, { 867,  60},
    { 740,  70}, { 669,  80}, { 595,  90}, { 541, 100}, { 397, 120}, { 248, 140},
};
// ── FRONT-LEFT sensor ──
static constexpr _SharpPt SHARP_LUT_FRONT_L[] = {
    {1810,  20}, {1439,  30}, {1061,  40}, { 894,  50}, { 777,  60},
    { 677,  70}, { 599,  80}, { 542,  90}, { 467, 100}, { 334, 120}, { 167, 140},
};
// ── LEFT sensor ──
// NOTE: 70→80 mm is nearly flat (770→766) and the far end sits high vs the
// other three — re-calibrate LEFT's 60-140 mm range if its readings look off.
static constexpr _SharpPt SHARP_LUT_LEFT[] = {
    {1784,  20}, {1390,  30}, {1102,  40}, { 934,  50}, { 822,  60},
    { 770,  70}, { 766,  80}, { 724,  90}, { 674, 100}, { 584, 120}, { 508, 140},
};

// All four LUTs share the same length.
static constexpr int SHARP_LUT_SIZE = sizeof(SHARP_LUT_RIGHT) / sizeof(_SharpPt);

// Compile-time guard: every LUT must have exactly SHARP_LUT_SIZE rows.
static_assert(sizeof(SHARP_LUT_FRONT_R) / sizeof(_SharpPt) == SHARP_LUT_SIZE,
              "SHARP_LUT_FRONT_R must have SHARP_LUT_SIZE rows");
static_assert(sizeof(SHARP_LUT_FRONT_L) / sizeof(_SharpPt) == SHARP_LUT_SIZE,
              "SHARP_LUT_FRONT_L must have SHARP_LUT_SIZE rows");
static_assert(sizeof(SHARP_LUT_LEFT) / sizeof(_SharpPt) == SHARP_LUT_SIZE,
              "SHARP_LUT_LEFT must have SHARP_LUT_SIZE rows");

// Per-sensor lookup — index matches WallSensorID_t (RIGHT=0 .. LEFT=3).
static constexpr const _SharpPt* SHARP_LUTS[4] = {
    SHARP_LUT_RIGHT,    // 0 = WALL_SENSOR_RIGHT
    SHARP_LUT_FRONT_R,  // 1 = WALL_SENSOR_FRONT_RIGHT
    SHARP_LUT_FRONT_L,  // 2 = WALL_SENSOR_FRONT_LEFT
    SHARP_LUT_LEFT,     // 3 = WALL_SENSOR_LEFT
};
 ...
 */
inline uint8_t _sharp_mv_to_mm(const _SharpPt* lut, uint16_t mv) {
    // mV ต่ำกว่า entry สุดท้าย = วัตถุอยู่ไกลเกิน range หรือไม่มีวัตถุ
    if (mv < lut[SHARP_LUT_SIZE - 1].mv) return 255;

    // mV สูงกว่า entry แรก = วัตถุชิดเกินไป → clamp ที่ระยะต่ำสุด
    if (mv >= lut[0].mv) return (uint8_t)lut[0].mm;
    ...
    for (int i = 0; i + 1 < SHARP_LUT_SIZE; i++) {
        if (mv <= lut[i].mv && mv >= lut[i + 1].mv) {
            // t = สัดส่วนที่ mv อยู่ระหว่าง lut[i] และ lut[i+1]
            float t  = (float)(lut[i].mv - mv) /
                       (float)(lut[i].mv - lut[i + 1].mv);
            float mm = (float)lut[i].mm +
                       t * ((float)lut[i + 1].mm - (float)lut[i].mm);
...
inline uint8_t wall_sensor_read(WallSensorID_t sensor_id) {
    ...
    // ขั้นที่ 2: แปลง mV → mm ผ่าน LUT เฉพาะของเซนเซอร์ตัวนี้ (per-sensor)
    // raw_mm = 255 หมายถึง mV ต่ำเกินไป = ไม่มีวัตถุหรืออยู่ไกลเกิน range
    uint8_t  raw_mm   = _sharp_mv_to_mm(SHARP_LUTS[sensor_id], mv);
