...
CHUNK_TARGET_SIZE = 2000  # ตรงกับ Rust smart_chunk_text() ใน ai_chat.rs
CHUNK_OVERLAP     = 100   # overlap characters between consecutive chunks
SUPPORTED_EXTS    = {".md", ".txt", ".c", ".h"}
INDEX_FILE_V1     = ".embeddings.json"       # schema v1 (legacy)
INDEX_FILE_V2     = ".knowledge_index.json"  # schema v2 (new)
OPENAI_MODEL      = "text-embedding-3-small"  # 1536-dim
GEMINI_MODEL      = "text-embedding-004"      # 768-dim
...
class GeminiBackend(EmbeddingBackend):
    ...
    Dimension: 768.  Best for Thai + English multilingual content.
    Requires: pip install requests
    Usage: python build_rag_index.py --gemini-key AIza...
    """
    ...
    def embed(self, text: str) -> list[float]:
        ...
        if not values:
            raise ValueError("Empty embedding values from Gemini")
...
def load_index(index_path: Path) -> dict:
    """Load existing .embeddings.json (v1) or return a fresh empty index."""
    if index_path.exists():
        try:
            ...
            print(f"  ⚠ Warning: corrupted {index_path.name}, starting fresh.")
    return {"chunks": [], "last_indexed": {}}


def detect_category(rel_key: str) -> str:
    """Mirror Rust detect_category() in ai_chat.rs."""
    if rel_key.startswith("sensor_examples/") or "sensor" in rel_key:
        return "sensor_examples"
    if rel_key.startswith("wiki/hardware/") or "pinout" in rel_key or "hardware" in rel_key or "kidbright32" in rel_key:
        return "hardware_reference"
    if rel_key.startswith("wiki/projects/") or any(k in rel_key for k in ["minibike", "micromouse", "skate", "formula"]):
        return "workshop_activities"
    if rel_key.startswith("wiki/"):
        return "ide_guides"
    return "general"


def generate_summary(text: str) -> str:
    """Mirror Rust generate_summary() — UTF-8 safe (Python handles this natively)."""
    for line in text.splitlines():
        t = line.strip()
        if t.startswith("### "): return t[4:].strip()[:120]
        if t.startswith("## "):  return t[3:].strip()[:120]
        if t.startswith("# "):   return t[2:].strip()[:120]
    # First 2 sentences
    count = 0
    result = []
    for ch in text:
...
def save_index_v2(kb_path: Path, v2_index: dict) -> None:
    """Write .knowledge_index.json (schema v2) atomically."""
    import datetime
    v2_index["indexed_at"] = datetime.datetime.utcnow().isoformat() + "Z"
    index_path = kb_path / INDEX_FILE_V2
    tmp_path   = kb_path / (INDEX_FILE_V2 + ".tmp")
    with open(tmp_path, 'w', encoding='utf-8') as f:
        json.dump(v2_index, f, ensure_ascii=False, indent=2)
    tmp_path.replace(index_path)
    size_kb = index_path.stat().st_size / 1024
    print(f"\n  [saved v2] {index_path} ({size_kb:.1f} KB)")
...
def build_index_v2(
    ...
    """
    Build .knowledge_index.json (schema v2) with category + chunk metadata.
    Matches the KnowledgeIndexV2 struct in ai_chat.rs exactly.
    """
    index_path = kb_path / INDEX_FILE_V2
    # Load existing v2 or start fresh
    ...
    provider_name = backend.name().split()[0].lower() if backend else "none"
    model_name    = getattr(backend, 'model', '') or getattr(backend, 'model_name', '')
    dim           = 0

    v2.setdefault("version",            "2.0")
    v2.setdefault("embedding_provider", provider_name)
    v2.setdefault("embedding_model",    model_name)
    v2.setdefault("embedding_dimension",0)
    v2.setdefault("total_chunks",       0)
    v2.setdefault("categories",         {})
    v2.setdefault("file_index",         {})

    all_files = collect_kb_files(kb_path)
    if not all_files:
        print(f"  No supported files found in {kb_path}")
        return v2, {"skipped": 0, "indexed": 0, "chunks_added": 0, "errors": 0}

    print(f"\n  Found {len(all_files)} file(s) in knowledge_base/")
    print(f"  Embedding backend: {backend.name() if backend else 'DRY RUN'}")
    print(f"  Schema: v2 → {INDEX_FILE_V2}")
    print(f"  Chunk size: {chunk_size} chars, overlap: {overlap} chars")
    print()
    ...
    for filepath, rel_key in all_files:
        mtime     = get_mtime(filepath)
        file_info = v2["file_index"].get(rel_key, {})
        cached_hash = file_info.get("file_hash", "")

        # Read content first to compare hash
        try:
            content = filepath.read_text(encoding='utf-8', errors='replace')
        except Exception as e:
            print(f"     [!] Read error: {e}")
        ...
        print(f"  [+] Indexing: {rel_key}")
        category = detect_category(rel_key)
        chunks   = chunk_text(content, chunk_size, overlap)
        print(f"     → {len(chunks)} chunk(s) [{category}]", end="")
        ...
        # Generate embeddings
        cat = v2["categories"].setdefault(category, {
            "description": f"Files in {category}",
            "chunks": []
        })
        # Remove old chunks for this file
        cat["chunks"] = [c for c in cat["chunks"] if c["source_file"] != rel_key]
        ...
        for i, chunk_content in enumerate(chunks):
            chunk_id = f"{rel_key.replace('/', '_').replace('.', '_').replace('-', '_')}_chunk_{i+1}"
            chunk_ids.append(chunk_id)
            embedding    = []
            needs_embed  = False
            for attempt in range(3):
                try:
                    embedding = backend.embed(chunk_content)
                    print(".", end="", flush=True)
                    time.sleep(0.05)
                    break
                except Exception as e:
            ...
            cat["chunks"].append({
                ...
                "summary":        generate_summary(chunk_content),
        ...
        print(f"\n     [OK] {success_count}/{total} chunks embedded" +
              (f", {failed_count} failed (needs_reindex)" if failed_count else ""))
        stats["indexed"]      += 1
        stats["chunks_added"] += success_count
        if failed_count:
            stats["errors"] += 1

    # Update totals
    v2["embedding_dimension"] = dim
    v2["total_chunks"] = sum(
        len(c["chunks"]) for c in v2["categories"].values()
    )
    return v2, stats
...
Examples:
  ...
  python scripts/build_rag_index.py --openai-key sk-... --query "LM73 temperature sensor I2C"
    ...
    backend_group.add_argument("--gemini-key", metavar="API_KEY",
                               help="Google Gemini API key (uses text-embedding-004, 768-dim)")
    ...
    if args.dry_run:
    ...
    elif hasattr(args, 'gemini_key') and args.gemini_key:
        try:
            import requests
        except ImportError:
            print("ERROR: requests not installed. Run: pip install requests")
            sys.exit(1)
        backend = GeminiBackend(api_key=args.gemini_key)
    elif args.local:
        try:
            import requests
        except ImportError:
        ...
        print("  To embed, use --openai-key, --gemini-key, --local, or --sentence-transformers\n")
        args.dry_run = True
    ...
    # ── Run indexing (v2 schema if backend is set) ─────────────────────────────
    start_time = time.time()
    use_v2 = backend is not None and not args.dry_run

    if use_v2:
        # Build .knowledge_index.json (schema v2)
        v2_index, stats = build_index_v2(
            kb_path=kb_path,
            backend=backend,
            force=args.force,
            dry_run=False,
        ...
        # Dry-run or legacy: build .embeddings.json (schema v1)
    ...
    if not args.dry_run:
        if use_v2:
            save_index_v2(kb_path, index)
        else:
            save_index(kb_path / INDEX_FILE_V1, index)
